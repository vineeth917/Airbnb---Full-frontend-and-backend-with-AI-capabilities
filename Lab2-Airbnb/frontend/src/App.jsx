...
function App() {
    ...
    const checkAuthStatus = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setCurrentUser(data.user);
          setIsLoggedIn(true);
          dispatch(setCredentials({ user: data.user, token: auth.token || null }));
          setCurrentView(data.user.user_type === 'owner' ? 'today' : 'homes');
  ...
  // Sync Redux auth state into local UI state (for current screens)
  useEffect(() => {
    if (auth?.isAuthenticated && auth.user) {
      ...
      setIsLoggedIn(true);
      if (!currentView) {
        setCurrentView(auth.user.user_type === 'owner' ? 'today' : 'homes');
      }
