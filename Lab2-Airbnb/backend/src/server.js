...
const { clientPromise, MONGO_DB_NAME } = require('./utils/mongo');
const { issueToken, verifyToken, revokeToken } = require('./utils/authTokens');
const { sendEvent } = require('./utils/kafka');

const app = express();
const PORT = process.env.PORT || 5000;

// Database configuration (env-driven so Docker/local stay in sync)
const dbConfig = {
...
// Session store (Mongo for central session/JWT persistence)
...
// Kafka booking notifications
const KAFKA_TOPIC_BOOKINGS = process.env.KAFKA_TOPIC_BOOKINGS || 'booking.events';
const sendBookingEvent = async (eventType, payload) => {
  try {
    await sendEvent(KAFKA_TOPIC_BOOKINGS, {
      event_type: eventType,
      emitted_at: new Date().toISOString(),
      ...payload
    });
  } catch (error) {
    console.error('Booking event publish failed:', error.message);
  }
};

// CORS (allow local/hosted frontends; configurable via CORS_ORIGINS)
...
app.use(cors({
  origin: (origin, callback) => {
    ...
    // Allow requests from ALB (check if origin contains elb.amazonaws.com)
    if (origin.includes('elb.amazonaws.com')) {
      return callback(null, true);
    }
    // Allow requests from same origin (when proxied through nginx)
    if (origin.includes('lab2-frontend-service') || origin.includes('localhost')) {
      return callback(null, true);
    }
...
const requireAuth = async (req, res, next) => {
  try {
    if (req.session.userId) {
      req.auth = { userId: req.session.userId, userType: req.session.userType };
      return next();
    }
    ...
    req.auth = { userId: decoded.userId, userType: decoded.userType, tokenId: decoded.jti };
    return next();
  } catch (error) {
...
const requireOwner = async (req, res, next) => {
  try {
    ...
    const userType = req.session.userType || req.auth?.userType;
    if (!req.session.userId || userType !== 'owner') {
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    let token;
    try {
      const issued = await issueToken(userId, userType);
      token = issued.token;
    } catch (tokenError) {
      console.error('Failed to issue JWT for new user:', tokenError.message);
    }
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    let token;
    try {
      const issued = await issueToken(user.id, user.user_type);
      token = issued.token;
    } catch (tokenError) {
      console.error('Failed to issue JWT on login:', tokenError.message);
    }
...
app.post('/api/auth/logout', async (req, res) => {
  ...
  if (bearer) {
    try {
      ...
      console.error('Failed to revoke token on logout:', tokenError.message);
    }
  }
...
app.route('/api/bookings')
  .post(requireAuth, async (req, res) => {
    try {
      const { listing_id, check_in, check_out, total_price, special_requests } = req.body;
      ...
      await pool.query(
        `INSERT INTO bookings (id, listing_id, guest_id, host_id, check_in, check_out, total_price, special_requests, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending')`,
        [bookingId, listing_id, req.session.userId, listings[0].host_id, check_in, check_out, total_price, special_requests || null]
      );
      ...
      // Notify Kafka about new booking
      await sendBookingEvent('booking.created', {
  ...
  .get(requireAuth, async (req, res) => {
    try {
      ...
      const isHostView = req.session.userType === 'owner' || as_host === 'true';
...
// Host: get upcoming bookings across their listings (pending or confirmed, future check-in)
app.get('/api/owner/bookings/upcoming', requireOwner, async (req, res) => {
  try {
    ...
    console.error('Get upcoming host bookings error:', error);
    res.status(500).json({
      success: false,
...
app.post('/api/bookings/:id/accept', requireOwner, async (req, res) => {
  try {
    // Get booking with host info - use b.host_id from bookings table
    const [bookings] = await pool.query(
      `SELECT b.*, b.host_id as booking_host_id, l.host_id as listing_host_id
       FROM bookings b
       JOIN listings l ON b.listing_id = l.id
       WHERE b.id = ?`,
      [req.params.id]
    ...
    // Use booking_host_id (from bookings table) for authorization check
    if (booking.booking_host_id !== req.session.userId) {
      return res.status(403).json({
        success: false,
    ...
    await sendBookingEvent('booking.confirmed', {
      booking_id: req.params.id,
      listing_id: booking.listing_id,
      guest_id: booking.guest_id,
      host_id: booking.booking_host_id,
...
app.post('/api/bookings/:id/cancel', requireAuth, async (req, res) => {
  try {
    ...
    await sendBookingEvent('booking.cancelled', {
      booking_id: req.params.id,
      listing_id: booking.listing_id,
      guest_id: booking.guest_id,
